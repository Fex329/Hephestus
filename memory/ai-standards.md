...
| **G6 — Domain review** | Domain specialist posts independent review findings, runs full test suite on branch, then merges feature branch to `develop` (site) or `main` (tooling/governance) via `--no-ff`. Merge only if G5 and G6 both approved in the same review cycle. After merge: (1) remove the worktree if one exists (`git worktree remove --force <path>`); (2) delete the remote feature branch (`git push origin --delete feature/iss-NNN-...`); (3) delete the local feature branch (`git branch -d feature/iss-NNN-...`). | Owner acknowledges — **blocker** |
